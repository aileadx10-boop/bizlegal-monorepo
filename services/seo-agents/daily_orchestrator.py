...
def task_00_brain_followup() -> dict:
    ...
    rows = supabase_query('daily_gaps', 'select=slug,status&order=created_at.desc&limit=20')
...
def task_08_site_health() -> dict:
    ...
    if err > 0:
        telegram_alert(f'⚠ Site health: {err}/{len(results)} endpoints have errors. Check decisions/.')
    return summary
...
def task_15_sales_attribution() -> dict:
    """15:00 UTC: Pull Stripe + NOWPayments + PayPal sales, attribute to blog UTM."""
    # NOWPayments (live)
    np_sales = []
    if NOWPAYMENTS_KEY:
        try:
            ...
            # /v1/payments is per-payment; /v1/list_payments may differ
            pass
        except Exception as e:
            print(f'  [sales] NOWPayments: {e}')

    summary = {
        'nowpayments_count_24h': len(np_sales),
        'nowpayments_total_usd': sum(float(p.get('price_amount', 0) or 0) for p in np_sales if p.get('payment_status') == 'finished'),
        'paypal_count_24h': 0,  # PayPal 401 - dead
        'stripe_count_24h': 0,   # Stripe vault key invalid
        'note': 'NOWPayments-only mode. PayPal and Stripe are offline until credentials refreshed.',
    }
