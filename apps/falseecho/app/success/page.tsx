...
function SuccessContent() {
  ...
  const reportId     = params.get('report')       // FalseEcho order report_id
  ...
  useEffect(() => {
    if (method === 'paypal' && paypalToken && reportId) {
      ...
      // NOWPayments — payment confirmed on their end, webhook fires the scan
      setStatus('done')
    }
    ...
    async function capturePayment() {
      try {
        ...
        if (!res.ok || !data.success) {
          ...
          if (data.scanRef) setScanRef(data.scanRef)
          setStatus('done')
        }
      } catch {
        ...
        <div style={{ maxWidth: 560, width: '100%' }}>
          <div style={{ padding: '36px 40px', background: C.greenBg, border: `1px solid rgba(39,174,96,0.25)`, borderRadius: 16, textAlign: 'center', marginBottom: 24 }}>
            ...
            <p style={{ fontSize: 15, color: C.muted, lineHeight: 1.65, marginBottom: 0 }}>
              Your 4-engine probe battery is running. The hash-anchored
              evidence pack lands in your inbox within minutes.
            </p>
          </div>
          ...
          {reportId && (
            <div style={{ padding: '20px 24px', background: C.surface, border: `1px solid ${C.border}`, borderRadius: 12, marginBottom: 20, textAlign: 'center' }}>
              ...
              <div style={{ fontSize: 12, color: C.dim, marginTop: 6 }}>Save this ID — it tracks your evidence pack</div>
            </div>
          )}

          <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap' }}>
            {scanRef && (
              <a href={`/report/${scanRef}`} style={{
          ...
          <div style={{ marginTop: 28, padding: '24px 28px', background: C.amberBg, border: `1px solid ${C.amber}33`, borderRadius: 12, textAlign: 'center' }}>
            <div style={{ fontSize: 14, fontWeight: 600, color: C.text, marginBottom: 6 }}>Engines change their answers daily</div>
            <p style={{ fontSize: 13, color: C.muted, marginBottom: 16, lineHeight: 1.6 }}>
              FalseEcho Monitor re-scans your name every day and emails you the moment a new falsehood appears — $149/mo, cancel anytime.
            </p>
            <a href="/pricing"
