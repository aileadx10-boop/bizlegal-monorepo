 ...
 * TRACR /api/paypal-capture). The /success page calls this with the PayPal
 * order token after the buyer returns from approval.
 */
export async function POST(req: NextRequest) {
  try {
    ...
    const { data: order } = await supabaseAdmin
      ...
      .select('id, report_id, scan_id, email, tier')
      .single()
    ...
    return NextResponse.json({ success: true, reportId, scanRef })
  } catch (err: unknown) {
