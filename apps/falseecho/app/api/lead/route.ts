 ...
 * falseecho_leads, enqueued into the fleet nurture cadence.
 */
export async function POST(req: NextRequest) {
  try {
    ...
    const { error } = await supabaseAdmin.from('falseecho_leads').insert({
      email,
      name,
      scenario,
      source,
    ...
    void enqueueNurture({
      lead_id: `falseecho-${Date.now()}`,
      email,
      vertical: 'falseecho',
      source: `falseecho:${source}`,
