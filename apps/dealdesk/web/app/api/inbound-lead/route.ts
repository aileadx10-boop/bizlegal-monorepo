...
const ACCEPTED_PRODUCTS: readonly string[] = ['dealdesk', 'leaseparse', 'closeflow']

/**
 * CloseFlow /api/inbound-lead — see docai equivalent for protocol.
 * Verifies HMAC-SHA256 of body against BIZLEGAL_INBOUND_SECRET.
 * Log-only accept (no nurture enqueue in the scaffold).
 */
...
export async function POST(req: NextRequest): Promise<NextResponse> {
  ...
  // DealDesk is one app serving two hands, so it accepts leads classified
  // under either former surface as well as its own name.
  if (!ACCEPTED_PRODUCTS.includes(payload.classification?.product ?? '')) {
    return NextResponse.json(
      { error: 'wrong_product', expected: ACCEPTED_PRODUCTS, received: payload.classification?.product ?? 'unknown' },
      { status: 400 }
    )
  }
...
export async function GET(): Promise<NextResponse> {
  return NextResponse.json(
    { ok: true, service: 'dealdesk', endpoint: 'inbound-lead', configured: Boolean(process.env.BIZLEGAL_INBOUND_SECRET) },
    { status: 200 }
  )
}
