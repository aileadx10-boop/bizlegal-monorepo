...
export async function POST(req: NextRequest): Promise<NextResponse> {
  ...
  if (payload.classification?.product !== 'forge') {
    return NextResponse.json(
      { error: 'wrong_product', expected: 'forge', received: payload.classification?.product ?? 'unknown' },
      { status: 400 }
    )
  }

  const leadId = payload.lead?.lead_id ?? 'unknown'
  console.log(`[inbound-lead] Forge received lead=${leadId} confidence=${payload.classification.confidence}`)
  ...
  logEventAsync({
    ...
    status: 'ok',
    metadata: {
      classification_product: payload.classification?.product,
...
export async function GET(): Promise<NextResponse> {
  return NextResponse.json(
    { ok: true, service: 'forge', endpoint: 'inbound-lead', configured: Boolean(process.env.BIZLEGAL_INBOUND_SECRET) },
    { status: 200 }
  )
}
