...
The currently-LIVE revenue subdomain. Hosts the State Transparency Report Kit ($149 — repositioned from the federal BOI product after FinCEN's 2025 rule ended BOI filing for US domestic companies), Regulatory Passport ($297), wallet scan ($97), and the gap-page lead magnets.
...
- `/boi` — State Transparency Report Kit ($149 one-time; route slug kept for SEO/link continuity)
...
2. `app/boi/success/page.tsx` has cross-sell to `/agents/boi-tracker` on the hub. Don't remove without Moses approval — it's the conversion bridge from Forge one-time to BOI Tracker subscription. (Note: the tracker copy still pitches federal BOI monitoring — flagged for Moses post-reposition.)
