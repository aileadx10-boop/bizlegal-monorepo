/**
 * Shared PayPal capture step for lexaudit card checkout — 2026-09-14.
 *
 * Port of apps/hub/lib/payments/paypal-capture.ts. Closes the same defect
 * on this surface: /api/payments/paypal/start creates PayPal orders with
 * intent=CAPTURE but nothing ever called /v2/checkout/orders/{id}/capture,
 * so buyers approved payment, no money moved, and the webhook marked the
 * order active on CHECKOUT.ORDER.APPROVED.
 *
...
// must hit the same environment that created the order (passed as an
// explicit override; the package's own default is PAYPAL_API_URL → live).
 ...
 *                      against payment_orders.gateway_subscription_id, where
 *                      start/route.ts stores the PayPal order id.
 */
export async function capturePayPalOrderForApp(opts: {
  orderId: string
  ...
  const { data: row, error: readErr } = await supabase
    .from('payment_orders')
    .select('id, status, gateway_subscription_id, metadata')
    .eq('id', opts.orderId)
    .maybeSingle()
  ...
  const storedPayPalId = order.gateway_subscription_id
