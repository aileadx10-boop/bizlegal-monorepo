 ...
 * presence boolean per critical secret on LexAudit. Names + presence
...
const ENV_KEYS: ReadonlyArray<{ name: string; critical: boolean; reason: string }> = [
  { name: 'NEXT_PUBLIC_SUPABASE_URL',     critical: true,  reason: 'compliance_subs + framework_index reads' },
  { name: 'SUPABASE_SERVICE_KEY',         critical: true,  reason: 'compliance + cert table writes' },
  ...
  { name: 'ANTHROPIC_API_KEY',            critical: true,  reason: 'compliance health score + Q2 semantic-diff Sonnet' },
  { name: 'RESEND_API_KEY',               critical: true,  reason: 'cert-ready + monitor change alerts' },
  { name: 'NOWPAYMENTS_API_KEY',          critical: true,  reason: 'crypto checkout (Compliance Monitor $99/mo)' },
  ...
  { name: 'FIRECRAWL_API_KEY',            critical: false, reason: 'Q2 semantic-diff scrape (rotated key required)' },
  { name: 'CRON_SECRET',                  critical: true,  reason: 'monitor/diff cron auth' },
]
