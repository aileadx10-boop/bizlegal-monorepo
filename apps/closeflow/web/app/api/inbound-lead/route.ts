...
export async function POST(req: NextRequest): Promise<NextResponse> {
  ...
  if (payload.classification?.product !== 'closeflow') {
    return NextResponse.json(
      { error: 'wrong_product', expected: 'closeflow', received: payload.classification?.product ?? 'unknown' },
      { status: 400 }
    )
  }
...
export async function GET(): Promise<NextResponse> {
  return NextResponse.json(
    { ok: true, service: 'closeflow', endpoint: 'inbound-lead', configured: Boolean(process.env.BIZLEGAL_INBOUND_SECRET) },
    { status: 200 }
  )
}
