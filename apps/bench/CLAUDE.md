...
**Primary surfaces** (`web/app/`): `/` · `/benchmarks[/slug]` · `/methodology` · `/sample` · `/pricing` · `/audit/request` (client intake) · `/experts` (talent funnel) · `/report/[id]?t=` (token-gated) · `/api/checkout/start` (**LIVE** — checkout enabled (2026-08-23), forwards to hub /api/pay/start) · `/api/inbound-lead` (HMAC).
...
**Data:** `supabase/migrations/20260816_bench_schema.sql` (bench_intake/clients/engagements/experts/expert_applications/evaluations/reports — RLS, service-role only). Benchmark JSONs in `web/data/benchmarks/` are the moat: git-versioned, reviewed by Moses Dor, status active, ≤5 released items per set, the rest held out.
