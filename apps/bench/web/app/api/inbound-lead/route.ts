...
/**
 * Bench /api/inbound-lead — see docai/propsignal equivalents for protocol.
...
export async function POST(req: NextRequest): Promise<NextResponse> {
  ...
  if (payload.classification?.product !== 'bench') {
    return NextResponse.json(
      { error: 'wrong_product', expected: 'bench', received: payload.classification?.product ?? 'unknown' },
      { status: 400 },
    )
  }
  ...
  if (email) {
    void dbInsert('bench_intake', {
      email: email.toLowerCase(),
      source: 'inbound_lead',
      message: payload.classification.reason ?? null,
    })
  }
...
export async function GET(): Promise<NextResponse> {
  return NextResponse.json(
    { ok: true, service: 'bench', endpoint: 'inbound-lead', configured: Boolean(process.env.BIZLEGAL_INBOUND_SECRET) },
    { status: 200 },
  )
}
