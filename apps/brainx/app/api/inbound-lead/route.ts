...
/**
 * BrainX /api/inbound-lead — HMAC-verified handoff from the fleet
...
export async function POST(req: NextRequest): Promise<NextResponse> {
  ...
  if (payload.classification?.product !== 'brainx') {
    return NextResponse.json(
      { error: 'wrong_product', expected: 'brainx', received: payload.classification?.product ?? 'unknown' },
      { status: 400 }
    )
  }
...
export async function GET(): Promise<NextResponse> {
  return NextResponse.json(
    { ok: true, service: 'brainx', endpoint: 'inbound-lead', configured: Boolean(process.env.BIZLEGAL_INBOUND_SECRET) },
    { status: 200 }
  )
}
