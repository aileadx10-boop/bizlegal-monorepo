 ...
 * /api/properties
 *
 * POST — create a dd_properties row (the shared spine for leases + closings)
...
export async function GET(req: NextRequest): Promise<NextResponse> {
  ...
  const { data, error } = await db
    ...
    .select(`
      ...
      dd_leases (id, lease_type, confidence_score, engine, parsed_at, created_at),
      dd_transactions (id, transaction_type, closing_date, status, created_at)
    `)
