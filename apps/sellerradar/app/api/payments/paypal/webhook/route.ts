 ...
 * BILLING.SUBSCRIPTION.ACTIVATED. Without this route the $99/mo monitor took
...
export async function POST(req: NextRequest) {
  try {
    ...
    // custom_id is the SR-… report_id we set when creating the subscription.
    const reportId =
    ...
    switch (eventType) {
      case 'BILLING.SUBSCRIPTION.ACTIVATED': {
        await supabaseAdmin
          ...
          .eq('id', order.id)
        // Idempotent: fulfillPaidOrder claims paid_at on the report, so a
        // re-delivered ACTIVATED provisions nothing twice.
        const reportRef = await fulfillPaidOrder(order)
        return NextResponse.json({ ok: true, status: 'active', reportRef })
      }
      ...
      case 'BILLING.SUBSCRIPTION.PAYMENT.FAILED': {
        // Stop the weekly re-scan. The order row keeps status 'paid' — the
...
async function stopMonitors(order: {
  ...
  if (order.analysis_id) {
    await supabaseAdmin
      ...
      .eq('report_id', order.analysis_id)
    return
  }
  if (order.email) {
    await supabaseAdmin
