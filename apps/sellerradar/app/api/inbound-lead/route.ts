...
/**
 * SellerRadar /api/inbound-lead — HMAC-verified lead handoff from the fleet
 * lead router (same protocol as FalseEcho /api/inbound-lead).
 */
...
export async function POST(req: NextRequest): Promise<NextResponse> {
  ...
  if (payload.classification?.product !== 'sellerradar') {
    return NextResponse.json(
      { error: 'wrong_product', expected: 'sellerradar', received: payload.classification?.product ?? 'unknown' },
      { status: 400 }
    )
  }
  ...
  console.log(`[inbound-lead] SellerRadar received lead=${leadId} confidence=${payload.classification.confidence}`)
  ...
  if (email && leadId !== 'unknown') {
    void enqueueNurture({
      ...
      source: 'sellerradar:inbound-lead',
      lead_classification: {
...
export async function GET(): Promise<NextResponse> {
  return NextResponse.json(
    { ok: true, service: 'sellerradar', endpoint: 'inbound-lead', configured: Boolean(process.env.BIZLEGAL_INBOUND_SECRET) },
    { status: 200 }
  )
}
