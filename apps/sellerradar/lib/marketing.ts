 ...
 * /api/marketing/trigger (goal M.4 in-repo half).
 *
 * When a fee schedule change moves a seller's numbers, we hand the event
 * to the marketing content queue. n8n (external, goal M.2) turns it into
 * blog/LinkedIn/SEO output and calls back to /api/marketing/callback.
 *
 * Failures are swallowed — marketing must never break an analysis.
 *
 * Required envs:
