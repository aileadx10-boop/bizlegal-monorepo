...
/**
 * PropSignal /api/inbound-lead — see docai equivalent for protocol.
...
export async function POST(req: NextRequest): Promise<NextResponse> {
  ...
  if (payload.classification?.product !== 'propsignal') {
    return NextResponse.json(
      { error: 'wrong_product', expected: 'propsignal', received: payload.classification?.product ?? 'unknown' },
      { status: 400 },
    )
  }
...
export async function GET(): Promise<NextResponse> {
  return NextResponse.json(
    { ok: true, service: 'propsignal', endpoint: 'inbound-lead', configured: Boolean(process.env.BIZLEGAL_INBOUND_SECRET) },
    { status: 200 },
  )
}
