/**
 * DocAI Safe — PII redaction layer (port of apps/lexaudit/lib/safe/redact.ts,
 * behaviour unchanged; keep the two in sync).
 *
 * Replaces direct identifiers in user-supplied text BEFORE that text is
 * sent to Anthropic. Operates on the prompt content (contract text, deal
 * descriptions, questionnaire answers, redlines) — NOT on metadata fields
 * stored for the customer record (the stored input_data / lead email stay
 * raw; only the model-bound copy is redacted).
 *
 ...
 * AI. The DocAI /security page describes the limits.
 *
 * Strategy:
