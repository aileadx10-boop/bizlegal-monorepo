/**
 * Shared SQA seed knowledge base + generator for DocAI's questionnaire
 * drafting surfaces.
 *
 * Extracted from app/api/sqa/draft/route.ts so the paid SQA flow and the
 * /free-kb demo reuse ONE retrieval + generation engine (marathon goal
 * 3.2). Do not fork this into per-endpoint copies.
 *
 * The seed KB is intentionally small — minimal SOC 2 / CAIQ / SIG-Lite +
 * privacy items so a first draft can ground in real material rather than
 * always going out-of-scope. Firm-tier subscribers blend their uploaded
 * items on top (see lib/sqa/firm-kb.ts).
 *
 * KnowledgeBaseItem fields per lib/sqa/knowledge-base.ts:
 * id, source_name, source_url, jurisdiction, topics, text, last_updated.
 */

import Anthropic from "@anthropic-ai/sdk";
import type { GenerateCallable, KnowledgeBaseItem } from "./index";

export const MAX_QUESTION_LEN = 4000;

const NOW_ISO = new Date().toISOString();

export const SEED_KB: KnowledgeBaseItem[] = [
  {
...
export function buildGenerator(): GenerateCallable {
