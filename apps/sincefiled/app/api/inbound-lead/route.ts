...
/**
 * SinceFiled /api/inbound-lead — HMAC-verified handoff from the fleet
...
export async function POST(req: NextRequest): Promise<NextResponse> {
  ...
  if (payload.classification?.product !== 'sincefiled') {
    return NextResponse.json(
      { error: 'wrong_product', expected: 'sincefiled', received: payload.classification?.product ?? 'unknown' },
      { status: 400 }
    )
  }
  ...
  if (email && leadId !== 'unknown') {
    void enqueueNurture({
      ...
      email,
      vertical: 'sincefiled',
      source: 'sincefiled:inbound-lead',
      lead_classification: {
        confidence: payload.classification.confidence,
        reason: payload.classification.reason,
      },
...
export async function GET(): Promise<NextResponse> {
  return NextResponse.json(
    { ok: true, service: 'sincefiled', endpoint: 'inbound-lead', configured: Boolean(process.env.BIZLEGAL_INBOUND_SECRET) },
    { status: 200 }
  )
}
