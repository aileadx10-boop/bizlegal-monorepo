...
function pickVertical(product: string | null | undefined): NurtureVertical {
  ...
  if (p.includes('casepage') || p.includes('matter-status')) return 'casepage'
  if (p.includes('sincefiled')) return 'sincefiled'
  if (p.includes('brainx') || p.includes('opportunity-radar')) return 'brainx'
  if (p.includes('deal44') || p.includes('workflow44')) return 'deal44'
