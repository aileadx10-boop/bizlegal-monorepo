...
interface HeroProps {
  ...
  tight?: boolean
  /** Phase H — show animated mesh shimmer behind hero (auto-disabled by prefers-reduced-motion). */
  shimmer?: boolean
  /** Phase H — trust strip below CTAs (e.g. ["Practitioner-reviewed", "Source-cited", "Cancel anytime"]). */
...
export function Hero({
  ...
  return (
    <section
      className={`bl-hero-bg${shimmer ? ' bl-hero-shimmer' : ''}`}
      style={{
        position: 'relative',
      ...
      >
        ...
        <div style={{ maxWidth: imageSrc ? '100%' : '780px' }}>
          ...
          {trustBullets && trustBullets.length > 0 && (
            <div className="bl-trust-strip" aria-label="Trust signals">
              {trustBullets.map((b) => (
                <span key={b}>{b}</span>
              ))}
            </div>
