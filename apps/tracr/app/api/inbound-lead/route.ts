...
/**
 * TRACR /api/inbound-lead — see lexaudit equivalent for protocol.
...
export async function POST(req: NextRequest): Promise<NextResponse> {
  ...
  if (payload.classification?.product !== 'tracr') {
    return NextResponse.json(
      { error: 'wrong_product', expected: 'tracr', received: payload.classification?.product ?? 'unknown' },
      { status: 400 }
    )
  }
  ...
  console.log(`[inbound-lead] TRACR received lead=${leadId} confidence=${payload.classification.confidence}`)
...
export async function GET(): Promise<NextResponse> {
  return NextResponse.json(
    { ok: true, service: 'tracr', endpoint: 'inbound-lead', configured: Boolean(process.env.BIZLEGAL_INBOUND_SECRET) },
    { status: 200 }
  )
}
