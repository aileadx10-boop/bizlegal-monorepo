...
**2026-09-15 — the relight (read `decisions/REVENUE-MACHINE-RELIGHT-2026-09-15.md` first).** Program plan = `decisions/REVENUE-MACHINE-PLAN-V3-2026-09-14.md`. Verified on production the same day: hub serves the relight code (PayPal capture-before-fulfil, live model ids, DocAI PII redaction, unshadowed sitemap/robots); the seven new SKUs (`cp_*` CasePage, `sf_*` SinceFiled) return real gateway URLs from `/api/pay/start` on both rails; `casepage_waitlist` + `sf_*` tables exist in the fleet Supabase; O-027 social autopilot runs live (442 drafts → 12/day digest by email via the hub relay). **Real revenue is still $0** (`payment_orders` active = 1 simulated smoke row; `fc_orders` 0 paid). **Second pass, same day (§7 of the handoff):** Hetzner relit by script (claude-sonnet-5, Anthropic 200, cron diet, O-027 + digest v2 crons), Cloudflare DNS applied, casepage/sincefiled prod env synced, and plan phases **B1/B2/B3/B4, GP2 and P** landed on local `main` (24 commits ahead; every one direct-`tsc` clean, hub `next build` green). Blocked on Moses: **Supabase restart** (the fleet DB wedged at ~17:34 UTC after MCP DDL timeouts — every REST call 503 until then; 5 migrations wait on it), `git push`, FirmCited `vercel --prod` (Stage 1 intake still 404 in prod), Vercel git-connect + Root Directory + domains for casepage/sincefiled, and the real $490 buy (G0, 2026-09-20). `turbo typecheck` false-greens on this Windows box — trust only direct `tsc`.
...
- `decisions/workflows/page_factory.md` — WAT SOP for the programmatic-SEO page factory (plan v3 §P, built 2026-09-15). Tool = `services/seo-agents/page_factory.py` + `page_factory_matrix.json`; gate = `services/seo-agents/page_quality_gate.py` (extends the curator's `quality_gate.py`); render = `apps/hub/app/(seo)/[hub]/[slug]`. Matrix v1 = 1,216 rows across four hub sections (`compliance` / `solutions` / `glossary` / `playbooks`); text comes from the free tier only (Gemini → OpenRouter `:free`, **never Anthropic**); the gate is ≥600 words, ≥3 registry-resolved citations, ≥3 unique FAQs, ≥5 verified internal links, no boilerplate, no outcome guarantees, YMYL disclaimer present. The script never publishes — Moses promotes `review` → `published`. Weekly proof = `seo_pages` count by hub/status + `index_status` breakdown; index rate <40% = stop the factory and consolidate.
