# Master Funnel — From Traffic to Revenue (2026-04-28, updated 2026-04-29)

**Author:** Claude (Phase K of the autonomous plan)
**Audience:** Moses
**Frame:** Anti-poverty operations. ~$35-50/mo OpEx + 6-product fleet + hub Pro + 10 agents = ~98% gross margin. One $25K OCI realestate close = 5 months of runway. We are not chasing ads; we are stacking organic + asymmetric.

## What landed in the L+M+H sprint (2026-04-28 → 2026-04-29)

Across two engineering days, 17 PRs merged across 6 repos:

**Phase L — 4 new agents** (10 total on /agents now):
- L1 DPA Negotiator (DocAI add-on, $69/mo) — `docai-monorepo#5`
- L2 PSP & MoR Risk Manager (hub-native, $99/mo) — `bizlegal-ai#34`
- L3 CTA-2024 BOI Tracker (Forge add-on, $29/mo) — `bizlegal-ai#32`
- L4 Compliance Monitor Pro (LexAudit add-on, $99/mo) — `lexaudit#5` + `bizlegal-ai#35`
- 4 agents added to /agents AGENTS array — `bizlegal-ai#36`

**Phase M — Deepening DocAI SQA + LexAudit Safe**:
- M1 SQA Firm-tier KB upload + paywall — `docai-monorepo#6`
- M2 LexAudit Safe PII redaction wired — `lexaudit#6`
- M3 LexAudit dynamic gateway (PayPal) + Resend cert-ready email — `lexaudit#7`

**Phase H — Gritnova Direction A** on hub homepage — `bizlegal-ai#37`

**Funnel hooks**:
- Forge BOI Kit + Passport `/success` → BOI Tracker cross-sell — `forge#3`

**Refund policy** rewrite across 6 repos (cancel anytime; one-time deliveries non-refundable once produced unless damaged/defective): `bizlegal-ai#33`, `trcr#4`, `BRAI#3`, `lexaudit#4`, `docai-monorepo#4`, `leadforge-ai#3`.

**Decision docs**: `bizlegal-ai#31` (SEO_FIXES + AGENTS_BRAINSTORM + MASTER_FUNNEL), `bizlegal-ai#30` (sitemap +17 URLs).
...
```
  ...
  │  Forge BOI Kit  → BOI Tracker $29/mo (forge#3 LIVE)      │
  │  Forge Passport → BOI Tracker $29/mo (forge#3 LIVE)      │
  │  TRACR scan     → LexAudit health-score subscription     │
  │  TRACR scan     → BRAI Sanctions Pro $500                │
  │  Pro subscriber → /agents add-ons (10 agents now live)   │
  │  DocAI SQA free → DocAI Team $69/mo (DPA Negotiator inc) │
  │  DocAI Team     → DocAI Firm $99/mo (KB upload — M1 LIVE)│
  │  LexAudit Solo  → Boutique upgrade (firm onboarding)     │
  │  Any LexAudit   → Compliance Monitor Pro $99/mo (L4 LIVE)│
  │  Stripe rejected → /psp-risk Recovery $499 (L2 LIVE)     │
