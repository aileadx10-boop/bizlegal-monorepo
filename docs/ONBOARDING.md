...
- **Corrupted `.pnpm` store**: if installs fail with integrity/ENOENT errors inside `node_modules/.pnpm`, repair with `pnpm store prune` then delete the app's `node_modules` and reinstall. If a single package entry is corrupted, delete just that entry under `node_modules/.pnpm/` and re-run `pnpm install`. **Recurring on this machine:** the global pnpm store (v10) has corrupt entries for `next@14.2.35`, `react@18.3.1`, `loose-envify@1.4.0`, `scheduler@0.23.2` — every fresh `pnpm install` relinks them as empty dirs (symptoms: `Cannot find module './impl'` or `ReactCurrentDispatcher` undefined at build time). Run `bash ~/pnpm-repair/repair.sh` to re-extract known-good tarballs into the virtual store. Durable fix = repair/replace the global store; `pnpm store prune` did not help.
