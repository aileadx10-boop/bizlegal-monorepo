...
- [ ] **4. Forge card price fork — RESOLVED 2026-09-06.** The `/boi` page charged **$169** while pricing said $149; code now collapses everything to **$149** (both `amount=` values accepted server-side, so old links keep working). No action needed unless you want $169 instead — say so and it flips.
- [ ] **5. Stale nurture voice.** The worker's email sequence for forge's `boi` vertical still uses the old federal-BOI scare copy ($591/day) — decision-tree leads get outdated emails until the copy is updated (flagged for a future code pass, not your dashboard).
