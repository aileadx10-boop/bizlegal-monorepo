 ...
 *
 * MOVED here verbatim from apps/closeflow/web/lib/checklist-templates.ts
 * (now a re-export shim). The only change is the optional Calendar param,
 * which defaults to MON_FRI and therefore reproduces the original dates.
 *
...
import { MON_FRI, type Calendar } from '../calendar.js'
import { addBusinessDays, type TransactionType } from './date-calculator.js'
...
export function generateChecklist(
  ...
  const dated: DatedChecklistTask[] = BASE_TEMPLATES[type].map((task) => ({
    ...task,
    dueDate: addBusinessDays(closingDate, task.offsetBusinessDays, cal),
  }))
